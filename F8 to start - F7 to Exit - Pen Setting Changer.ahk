...
Global StartingFrequencySetting := 0.1
Global EndFrequencySetting := 200
...
WrongValue(a,b){
	MsgBox("Please enter a value between " %a% " - " %b%)
	Sleep 100
}
...
StartingMessage(a){
	...
	Else StartingPowerSettingMessage()
}	
...
PowerSettingMessage(){ ;Runs if SetupType == 1
	...
	msg := InputBox("Please enter the power (W) setting. `n" . MinimumPower . " - " . MaximumPower, "Power Setting", "w300 h150", 20)
		if msg.Result="Cancel"
			...
			If (msg.Value < MinimumPower OR msg.Value > MaximumPower)
			{
				Sleep 100
				WrongValue(MinimumPower,MaximumPower)
				PowerSettingMessage()
				}
				
			Else
				StartingPowerSetting:=msg.Value
...
Global MaximumPower
	...
	msg:=InputBox("Please enter the end power (W) setting. `n" StartingPowerSetting " - " MaximumPower, "End Power Setting", "w300 h150", 20)
		if msg.Result ="Cancel"
			...
			If (msg.Value <= StartingPowerSetting OR msg.Value > MaximumPower)
			{
				Sleep 100
				WrongValue(StartingPowerSetting,MaximumPower)
				EndPowerSettingMessage()
				}
				
			Else
				EndPowerSetting:=msg.Value
				StartingSpeedSettingMessage()
	}
...
StartingSpeedSettingMessage(){
	...
	msg:=InputBox("Please enter the speed (mm/s) setting to start from. `n" . MinimumSpeed . " - " . MaximumSpeed, "Starting Speed Setting", "w300 h160", 100)
		if msg.Result="Cancel"
			...
			If (msg.Value < MinimumSpeed OR msg.Value >= MaximumSpeed)
			{
				WrongValue(MinimumSpeed,MaximumSpeed)
				StartingSpeedSettingMessage()
				}
			else
				StartingSpeedSetting:=msg.Value
...
EndSpeedSettingMessage(){
	...
	msg:=InputBox("Please enter the end speed (mm/s) setting. `n" StartingSpeedSetting " - " MaximumSpeed,"End Speed Setting", "w300 h150",20000)
		...
		If (msg.Value <= StartingSpeedSetting OR msg.Value > MaximumSpeed)
			{
...
StartingFrequencySettingMessage(){
	...
	msg:=InputBox("Please enter the Frequency (kHz) setting to start from. `n" MinimumFrequency " - " MaximumFrequency,"Starting Frequency Setting", "w300 h175", 0.1)
		...
		If (msg.Value < MinimumFrequency OR msg.Value >= MaximumFrequency)
			{
				WrongValue(MinimumFrequency,MaximumFrequency)
				StartingFrequencySettingMessage()
				}	
		else
			StartingFrequencySetting:=msg.Value
...
EndFrequencySettingMessage(){
	...
	msg:=InputBox("Please enter the Frequency (kHz) setting to End with. `n" StartingFrequencySetting " - " MaximumFrequency,"End Frequency Setting", "w300 h175", 200)
		...
		If (msg.Value < StartingFrequencySetting OR msg.Value > MaximumFrequency)
			{
				WrongValue(StartingFrequencySetting,MaximumFrequency)
				EndFrequencySettingMessage()
				}
		else
			EndFrequencySetting:=msg.Value
...
DPMMMessage(){
	...
	msg:=InputBox("Please enter the dots per mm.`nIf unsure use`n33.33 for marking`n333.33 for cutting.`nDots per mm is the relationship between Frequency (kHz) and Speed (mm/s)","Dots Per mm", "w300 h220", 33.33)
		if msg.Result="Cancel"
			...
			DPMM:=msg.Value
			;GridHeightMessage() ;In case we want a variable Grid size
			StartMessage()
}
	
/*	
This section of code was used during testing, and is still here in case varying grid sizes were used... This is still easily changed at the top of the program
...
tText:=""
	...
	If WinExist("TRUMPF TruPulse nano (SPI-G4) Pen#")
	{
		WinActivate
		tText:=ControlGetChoice("ComboBox1")
		If tText == "2"
		{	
			MainFunction()
		}
...
Global ModF										;This function calculates the Frequency when it is tied to Speed during the DPMM constant
	If ((S * D)/1000 >= MaximumFrequency){		;section of this program.
...
MainFunction(){
	...
	If (EndPowerSetting > StartingPowerSetting){											;Encase of a backwards inputs...
	...
	If (EndSpeedSetting > StartingSpeedSetting)	{												;Same as above
	...
	Loop GridHeight{
		...
		If (SetupType== 1){					;Checks if we are using a constant Power
			ModF := StartingFrequencySetting	;Reset Frequency back to it's original
		}

		Loop GridWidth{
			If (SetupType==0){			;Checks if we are using a constant DPMM
			...
			Send "{Down}"					;Next Pen
			Sleep 1000					;There's a large delay in SAMLight when as it loads the next pen
			Send "{Enter}"					;This just removes the pen drop down, probably not needed
			ss()
		
			If (SetupType==0){							;Checks if we're using constant power or constant DPMM
